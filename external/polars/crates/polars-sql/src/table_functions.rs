...
use sqlparser::ast::{BinaryOperator, Expr, FunctionArg, FunctionArgExpr, FunctionArgOperator, Value};
...
impl PolarsTableFunctions {
    ...
    fn get_boolean_from_arg(&self, arg: &FunctionArgExpr) -> Option<bool> {
        match arg {
            FunctionArgExpr::Expr(Expr::Value(Value::Boolean(b))) => Some(*b),
            _ => None,
        }
    }

    fn get_string_from_arg(&self, arg: &FunctionArgExpr) -> Option<String> {
        match arg {
            FunctionArgExpr::Expr(Expr::Value(Value::SingleQuotedString(s))) => Some(s.clone()),
            _ => None,
        }
    }

    fn get_number_from_arg(&self, arg: &FunctionArgExpr) -> Option<usize> {
        match arg {
            FunctionArgExpr::Expr(Expr::Value(Value::Number(n, _))) => n.parse::<usize>().ok(),
            _ => None,
        }
    }
    ...
    fn read_csv(&self, args: &[FunctionArg]) -> PolarsResult<(String, LazyFrame)> {
        polars_ensure!(args.len() >= 1, SQLSyntax: "`read_csv` expects at least a single file path; found {:?} arguments", args.len());
        ...
        for arg in args.iter().skip(1) {
            match arg {
                // Case 1: Standard named argument (identifier-based)
                FunctionArg::Named { name, arg, operator } => {
                    named_args.push(FunctionArg::Named {
                        name: name.clone(),
                        arg: arg.clone(),
                ...
                // Case 2: Expression-based named argument (Dialect-dependent)
                FunctionArg::ExprNamed { name, arg, operator } => {
                    if let Expr::Identifier(ident) = name {
                        ...
                        polars_bail!(SQLSyntax: "Invalid named argument format: expected `identifier = value`.");
                    }
                }

                // Case 3: BinaryOp inside Unnamed (e.g. `separator = ','`)
                FunctionArg::Unnamed(FunctionArgExpr::Expr(Expr::BinaryOp { left, op, right })) if *op == BinaryOperator::Eq => {
                    if let (Expr::Identifier(ident), Expr::Value(value)) = (&**left, &**right) {
                        named_args.push(FunctionArg::Named {
                            name: ident.clone(),
                            arg: FunctionArgExpr::Expr(Expr::Value(value.clone())),
                            operator: FunctionArgOperator::Equals,
                        });
                    } else {
                        polars_bail!(SQLSyntax: "Invalid argument format: expected `name = value`.");
                    }
                }

                _ => polars_bail!(SQLSyntax: "Invalid argument format: expected named arguments."),
            }
        }
        ;
        ...
        for arg in &named_args {
            if let FunctionArg::Named { name, arg, operator } = arg {
                match name.value.to_lowercase().as_str() {
                    "separator" => {
                        if let Some(s) = self.get_string_from_arg(arg) {
                            let separator_char = if s == "\\t" { '\t' as u8 }
                            else if s.len() == 1 { s.as_bytes()[0] }
                            else {
                                polars_bail!(SQLSyntax: "Separator must be a single character or `\t` for TAB.");
                            };

                            reader = reader.with_separator(separator_char);
                    ...
                    "try_parse_dates" => {
                        if let Some(b) = self.get_boolean_from_arg(arg) {
                            reader = reader.with_try_parse_dates(b);
                        } else {
                    ...
                    "missing_is_null" => {
                        if let Some(b) = self.get_boolean_from_arg(arg) {
                            reader = reader.with_missing_is_null(b);
                        } else {
                            reader = reader.with_missing_is_null(true);
                        }
                    },
                    _ => polars_bail!(SQLSyntax: "unknown argument: {}", name),
                }
            } else {
                polars_bail!(SQLSyntax: "expected named arguments; found: {}", arg);
            }
        }
